...
-- Auto-runs on boot. Only job: walk back to (0,0,0)/heading 0 if
-- end_miner.lua left a saved position. Never auto-mines.
--
-- Standalone -- shares no modules with end_miner.lua on purpose.
-- Never digs ordinary terrain by default (SKIP_BLOCKS always routed
-- around) -- a stale saved position once dug through a player's base.
-- Run `startup force` to override and dig through, if you're SURE the
-- saved position is accurate.
--
-- Stuck -> logs and stops; clear it and rerun `startup` (or `startup
-- force`). STATE_FILE format must match end_miner.lua's saveState().
...
local SKIP_BLOCKS = (configOk and type(rawConfig) == "table" and rawConfig.SKIP_BLOCKS) or DEFAULT_SKIP_BLOCKS

local MAX_MOVE_ATTEMPTS = 8
local MAX_DIG_ATTEMPTS = 8 -- give up retrying a block after this many tries

-- No footprint to cap to here (unlike end_miner.lua) -- generous enough
-- for a normal obsidian vein, not so wide it probes forever.
local DETOUR_MAX_WIDTH = 16

local pos = { x = 0, y = 0, z = 0 }
local heading = 0

-- Set by the "force" argument (see header). Default false: ordinary
-- terrain blocks the walk instead of being dug.
...
-- Like log(), but also appended to LOG_FILE -- a stuck walk is usually
-- unattended. Write failure never stops the walk.
...
-- Keeps STATE_FILE in sync as the walk progresses, so a second
-- interruption resumes from here, not the original position.
...
-- true (clear), or false + "skip"/"blocked"/"stuck" + block name.
-- SKIP_BLOCKS never dug; other blocks dug only if allowDig (force).
...
-- Bounded retries; hard stop the instant fuel hits 0.
...
-- Swings out sideHeading, crosses back to originalHeading. Inlined copy
-- of detour.lua's logic (standalone script). Undoes steps on failure.
...
-- Drop-in for forward(): tries a detour on a real obstacle before
-- reporting failure. Fuel reasons pass through untouched.
...
-- x then z, re-corrected in rounds (same reasoning as movement.lua's
-- walkHorizontal()). Cycle check so a corner can't loop forever.
...
-- Horizontal first, vertical last (see header). Returns true if it
-- made it all the way to (0,0,0)/heading 0.
